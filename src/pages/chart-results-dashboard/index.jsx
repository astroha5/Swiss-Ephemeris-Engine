...
const ChartResultsDashboard = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `vedic-chart-analysis-${birthDetails?.name || 'chart'}.html`;
      ...
      console.log('Chart analysis downloaded successfully');
    } catch (error) {
  ...
  const generatePDFContent = () => {
    const name = birthDetails?.name || 'User';
    const date = birthDetails?.dateOfBirth || 'N/A';
    const time = birthDetails?.timeOfBirth || 'N/A';
    const place = birthDetails?.placeOfBirth || 'N/A';
    
    return `
      <!DOCTYPE html>
      <html>
      <head>
        <title>Vedic Chart Analysis - ${name}</title>
        <style>
          body { font-family: Arial, sans-serif; margin: 20px; }
          .header { text-align: center; margin-bottom: 30px; }
          .section { margin-bottom: 25px; }
          .section h2 { color: #333; border-bottom: 2px solid #e0e0e0; }
          .birth-details { background: #f5f5f5; padding: 15px; border-radius: 8px; }
        </style>
      </head>
      <body>
        <div class="header">
          <h1>Vedic Astrology Chart Analysis</h1>
          <p>Generated by Astrova</p>
        </div>
        <div class="section">
          <h2>Birth Details</h2>
        ...
        <div class="section">
          ...
          <p>This is a comprehensive Vedic astrology analysis based on your birth details.</p>
          <p>For detailed interpretations, please visit the full dashboard.</p>
        </div>
      </body>
      </html>
    `;
  ...
  const handleShareResults = async () => {
    try {
      const shareData = {
        title: `Vedic Chart Analysis - ${birthDetails?.name || 'User'}`,
        text: `Check out my Vedic astrology chart analysis generated by Astrova!`,
      ...
      const notification = document.createElement('div');
      notification.textContent = 'Failed to share results';
      notification.style.cssText = `
        position: fixed;
        top: 20px;
        right: 20px;
                  ...
                  <div className="bg-surface rounded-xl border border-border shadow-soft p-6">
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                      ...
                      <button 
                        ...
                        className="flex items-center justify-center space-x-2 p-4 bg-secondary text-secondary-foreground rounded-lg hover:bg-secondary/90 transition-celestial"
                      >
